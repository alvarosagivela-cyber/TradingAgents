...
def run_analysis(checkpoint: bool | None = None):
    ...
    with Live(layout, refresh_per_second=4):
        ...
        # Mark all selected analysts started for wall-time bookkeeping
        for key in selected_analyst_keys:
            analyst_wall_time_tracker.mark_started(key)

        # D-01: Use propagate() instead of manual state + stream()
        # This call internally handles checkpoint setup/teardown and is resumable.
        ...
        # Mark all selected analysts completed for wall-time bookkeeping
        for key in selected_analyst_keys:
            analyst_wall_time_tracker.mark_completed(key)
        ...
        message_buffer.add_message(
            "System", f"Completed analysis for {selections['analysis_date']} (signal={processed_signal})"
        )
